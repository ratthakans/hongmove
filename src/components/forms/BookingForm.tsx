...
export function BookingForm() {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div>
        <label className="mb-1.5 block text-sm font-medium text-ink/80">ต้องการรถเมื่อไหร่</label>
        <div className="flex flex-wrap gap-2">
          {[
            { k: "now", t: "จองทันที (รับรถเลย)" },
            { k: "schedule", t: "นัดวันเวลา" },
          ].map((o) => (
            <button
              type="button"
              key={o.k}
              onClick={() => setWhenMode(o.k as "now" | "schedule")}
              className={`rounded-full border px-4 py-2 text-sm transition-colors ${
                whenMode === o.k
                  ? "border-crimson bg-crimson text-white"
                  : "border-line bg-white text-muted hover:border-crimson/40"
              }`}
            >
              {o.t}
            </button>
